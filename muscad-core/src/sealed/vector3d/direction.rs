...
mod tests {
    ...
    fn test_v3d_reverse() {
        ...
        assert_eq!(vector3d::reverse(&u), [-1.0, -2.0, -3.0]);
    }

    #[test]
    fn test_v3d_reverse_mut() {
        let mut u = [1.0, 2.0, 3.0];

        vector3d::reverse_mut(&mut u);

        assert_eq!(u, [-1.0, -2.0, -3.0]);
    }

    #[test]
    fn test_v3d_is_parallel() {
        let u = [1.0, 2.0, 3.0];
        let v = [-1.0, -2.0, -3.0];
        let w = [-2.0, -4.0, -6.0];
        ...
        assert!(is_parallel(&u, &vector3d::zero()));
        assert!(is_parallel(&[0.0, 0.0, 0.0], &vector3d::zero()));
        assert!(!is_parallel(&u, &[f64::PI, 1.0, 2.0]));
    }

    #[test]
    fn test_v3d_is_same_direction() {
        let u = [1.0, 2.0, 3.0];
        let v = [-1.0, -2.0, -3.0];
        let w = [-2.0, -4.0, -6.0];
        ...
        assert!(is_same_direction(&u, &vector3d::zero()));
        assert!(is_same_direction(&[0.0, 0.0, 0.0], &vector3d::zero()));
        assert!(!is_same_direction(&u, &[f64::PI, 1.0, 2.0]));
    }
}
